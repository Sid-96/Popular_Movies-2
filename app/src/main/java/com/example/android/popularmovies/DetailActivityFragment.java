...
public class DetailActivityFragment extends Fragment {
    ...
    public LinearLayout trailers_linear,extras_linear, reviews_linear;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        detailFragmentView = rootView;
        trailers_linear = (LinearLayout) detailFragmentView.findViewById(R.id.trailersList);
        extras_linear = (LinearLayout) detailFragmentView.findViewById(R.id.extraList);
        reviews_linear = (LinearLayout) detailFragmentView.findViewById(R.id.reviewsList);
        scrollView = (ScrollView) detailFragmentView.findViewById(R.id.detailScrollView);
        return rootView;
    }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        RelativeLayout container = (RelativeLayout) detailFragmentView.findViewById(R.id.detailScrollViewContainer);
    ...
    public static DetailActivityFragment newInstance(Movies newMovie) {
    ...
    public void updateUI(){
        ...
        if (favStatus)
            ...
            fab.setImageDrawable(ContextCompat.getDrawable(getActivity(), R.drawable.fav_off));
        Picasso.with(getContext()).load(movie.image).
                placeholder(R.id.posterImageView).into((ImageView) detailFragmentView.findViewById(R.id.posterImageView));
        ((TextView) detailFragmentView.findViewById(R.id.overviewTextView)).setText(movie.overview);
        ((RatingBar) detailFragmentView.findViewById(R.id.rating)).setRating(movie.rating / 2f);
        ((TextView) detailFragmentView.findViewById(R.id.ratingTextView)).setText((float) Math.round(movie.rating*10d)/10d + "/10");

        SimpleDateFormat df = new SimpleDateFormat("dd MMM, yyyy");
        SimpleDateFormat dfInput = new SimpleDateFormat("yyyy-MM-dd");
        String releasedDate;
        try {
            releasedDate = df.format(dfInput.parse(movie.releaseDate));
        } catch (ParseException e){
            e.printStackTrace();
            releasedDate = movie.releaseDate;
        }
        ((TextView) detailFragmentView.findViewById(R.id.releaseDate)).setText(releasedDate);
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        for (int i = 0; i < trailerAdapter.getCount(); i++){
                            trailers_linear.addView(trailerAdapter.getView(i, null, null));
                        }
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            extra.originalLanguage=response.getString("original_language");
                            ...
                            extra.productionCompanies="";

                            for(int i = 0; i < jsonArray1.length(); i++)
                            {
                                JSONObject object = jsonArray1.getJSONObject(i);
                                extra.productionCompanies+=object.getString("name");
                                if(i!=jsonArray1.length()-1)
                                    extra.productionCompanies+=" ,";
                            }

                            extras_linear.addView(createExtraView(extra));
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                    }
    ...
    public View createExtraView(Extra extra){
        ...
        ((TextView) view.findViewById(R.id.original_language)).setText(extra.originalLanguage);
        ...
        ((TextView) view.findViewById(R.id.production_companies)).setText(extra.productionCompanies);
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            for (int i=0; i<items.length(); i++){
                                ...
                                reviews_linear.addView(view = createReviewView(review, i));
                                ...
                                scrollView.post(new Runnable() {
                                    ...
                                    public void run() {
                                        scrollView.scrollTo(0,
                                                ((scrollOverheadId>0) ? detailFragmentView.findViewById(scrollOverheadId).getTop() : 0) +
                                                        detailFragmentView.findViewById(scrollId).getTop());
                                    }
                                });
                            }
                        } catch (JSONException e){
